...
int main(int argc, char** argv)
{
    ...
    int iterations_without_progress_threshold = 20;
    if (argc == 3){
        std::istringstream ss(argv[2]);
        if (!(ss >> iterations_without_progress_threshold)) {
            iterations_without_progress_threshold = 20;
        } else if (!ss.eof()) {
            iterations_without_progress_threshold = 20;
        }
    }
    ...
    trainer.be_verbose();
    trainer.set_synchronization_file("chkpts/face_metric_sync", std::chrono::minutes(5));
    ...
    trainer.set_iterations_without_progress_threshold(iterations_without_progress_threshold);
    ...
    net.clean();
    serialize("weights/metric_network_renset_latest.dat") << net;
